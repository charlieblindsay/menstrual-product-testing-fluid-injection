...
while(True):
    ...
    print(f'Seconds since start: {(datetime.now() - start_time).total_seconds()} @ {now.hour}:{now.minute}:{now.second}, temperature = {last_read_temperature}, PID output = {last_read_PID_output}')
    time.sleep(5)